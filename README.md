...
| t3.medium 인스턴스 실패 | AWS 신규 계정의 Free Tier는 특정 인스턴스 타입만 허용 | `describe-instance-types` API로 Free Tier 적격 타입 목록을 직접 조회해서 t3.small 선택 | 인프라 설계 시 계정 제약 조건을 사전에 확인하는 것이 중요. `aws ec2 describe-instance-types` 활용법 습득 |
...
| Datadog API Key 오류 (403) | Application Key(`ddapp_`으로 시작)를 API Key로 혼동해서 Agent 인증 실패. `kubectl logs`로 403 에러 확인 | Datadog 콘솔 Organization Settings에서 올바른 API Key(32자리 hex) 발급 후 `helm upgrade`로 교체 | Datadog의 Key 종류를 명확히 구분하게 됨. API Key는 Agent 데이터 전송용, Application Key는 API 호출용으로 역할이 다름 |
